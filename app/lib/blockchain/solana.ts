...
/**
 * Fetch a parsed transaction with retry/backoff. Public Solana RPCs are flaky;
 * a single timeout should not fail a payment verification. Returns null when
 * the transaction is not found (or not yet confirmed at the current commitment).
 */
export async function getParsedTransactionWithRetry(
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      if (attempt < attempts) {
        console.warn(
          `[solana] getParsedTransaction attempt ${attempt}/${attempts} failed ` +
            `(${known.message || (err instanceof Error ? err.message : "unknown")}). Retrying…`,
        );
        await new Promise((r) => setTimeout(r, attempt === 1 ? 500 : 1250));
        continue;
      }
      throw err;
    }
...
/**
 * Find a native SOL transfer instruction whose destination is `recipient`
 * inside a parsed transaction. Returns the lamports sent and the payer.
 * System-program transfers surface as `parsed.type === "transfer"`.
 */
export function findSolTransferTo(
  tx: ParsedTransactionWithMeta | null,
  recipient: string,
): { lamports: number; source: string } | null {
  ...
  for (const ix of tx.transaction.message.instructions) {
    ...
    if (parsed?.type === "transfer") {
      const info = parsed.info as { destination?: string; lamports?: string | number; source?: string };
      if (info.destination === recipient) {
        return { lamports: Number(info.lamports ?? 0), source: info.source ?? "" };
      }
    }
  }
  return null;
