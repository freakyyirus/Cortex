...
import { Loader2, CheckCircle, Wallet, ArrowLeft, Sparkles, X } from "lucide-react";
...
import { getSolanaAddressExplorerUrl } from "@/app/lib/payout";
...
export default function BountyDetailPage({ params }: { params: { id: string } }) {
  ...
  const { connected, address: solanaAddress, connecting: solanaConnecting, selectWallet, sendSol } = useSolanaWallet();
  ...
  const getAllUserAddresses = (): string[] => {
    ...
    if (user?.wallet?.address) {
      addresses.add(user.wallet.address);
    }
    if (solanaAddress) {
  ...
  const isCreator = bounty && (
    ...
    allUserAddresses.some(addr => addr === bounty.creatorAddress)
  );

  const submissionAddress = solanaAddress;
  ...
  const handlePayWinner = async (submissionId: string, winnerAddress: string) => {
    ...
    try {
      const result = await sendSol(winnerAddress, bounty!.prize);

      if (!result?.hash) {
        alert("The SOL payout was not completed. Check your wallet signature request and try again.");
        return;
      }
    ...
    <div className="min-h-screen pt-24 pb-16">
      <div className="mx-auto max-w-4xl px-6">
              ...
              <form onSubmit={handleSubmitWork} className="space-y-8">
                ...
                {submissionAddress && (
                  <p className="text-center text-[10px] font-bold text-primary/30 uppercase tracking-widest">
                    ...
                    {" · "}
                    <a
                      href={getSolanaAddressExplorerUrl(submissionAddress)}
                      target="_blank"
              ...
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {isReviewing ? "Analyzing submissions with Gemini..." : isReviewDone ? "Submissions scored and top picks flagged." : "Preparing AI..."}
                  </p>
                </div>
              </div>
        ...
        <ConnectWalletPrompt
          ...
          isLoading={solanaConnecting}
          title={walletPromptAction === "pay" ? "Connect Wallet to Pay" : "Connect Wallet"}
          description={
            walletPromptAction === "pay"
              ? `You need to connect a Solana wallet with ${CURRENCY} tokens to pay the winner.`
              : "Connect your Solana wallet to submit work. Prize winnings will be sent to your connected wallet."
          }
        />
      </div>
    </div>
