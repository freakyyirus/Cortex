...
export default function BountiesPage() {
  ...
  useEffect(() => {
    fetch("/api/bounties")
      ...
      .catch((err) => {
        console.error(err);
        setFetchError("Could not load bounties. Check your connection or try again.");
        setLoading(false);
      });
  }, []);
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        {fetchError ? (
          <div className="border border-line bg-bg-elevated flex flex-col items-center justify-center py-24 text-center rounded-[24px]">
            <Search className="h-6 w-6 text-danger/40 mb-4" />
            <h3 className="text-sm font-semibold text-primary">Connection Error</h3>
            <p className="mt-1 text-xs font-medium text-primary/40 tracking-tight max-w-md">{fetchError}</p>
            <button
